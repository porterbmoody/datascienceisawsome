...
`tidyr::complete()` in R takes a set of columns, and finds all unique combinations. It then ensures the original dataset contains all those values, filling in explicit missingness where necessary. You can use the pandas functions `stack()` and `unstack()` with `set_index()` to perform a similar function.  See [this stackoverflow](https://stackoverflow.com/questions/44287445/pandas-or-python-equivalent-of-tidyr-complete) for an example.
